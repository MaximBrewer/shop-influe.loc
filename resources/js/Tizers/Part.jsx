...
export default (props) => {
    ...
    return <div className={`product-card`}>
        {!auth.user || auth.user.type === 'BUYER' ? <button className={`btn-plain product-card-favourite ${favorites.parts.indexOf(item.uniqid) > -1 ? `active` : ``}`}
            onClick={() => {
                favorite()
            }}>
        ...
        <div className={`product-card-price`}>
            ...
            {!auth.user || auth.user.type === 'BUYER' ? <a href="#" onClick={(e) => {
                e.preventDefault()
                addToCart()
